...
final class ActivityTimerActionView: UIView {
    ...
    private var currentTimerState: ActivityTimerState = .none
    ...
    private func applyTheme() {
        var configuration = recordButton.configuration ?? UIButton.Configuration.filled()
        configuration.title = "Record"
        configuration.image = UIImage(systemName: "plus.circle.fill")
        configuration.imagePadding = 6
        configuration.buttonSize = .small
        ...
        configuration.titleTextAttributesTransformer = UIConfigurationTextAttributesTransformer { attributes in
            ...
            updatedAttributes.font = AppTheme.roundedFont(ofSize: 15, weight: .semibold)
